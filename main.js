...
ipcMain.handle('get-tables', async (event, dbName, tableName, limit, startTime, endTime) => {
  console.log('🔍 get-tables called with:', { dbName, tableName, limit, startTime, endTime });
  try {
...
ipcMain.handle('get-mssql-tables', async () => {
  ...
  try {
    ...
    console.log('✅ get-mssql-tables result length:', result?.length || 0);
    return result;
  } catch (error) {
